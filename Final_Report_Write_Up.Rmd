...
###Least Squares
The least squares regression method that was tested was the best subset selection. The methodology used to determine the best subset model was to first run cross validation on the training set in order to determine the number of predictors to include in the model. Once this analysis indicated that any added predictor after 3 variables were selected did not increase the accuracy of the model greatly using this cross validated method.  The training data was then used to determined the best subset of the linear model with three predictors. The best subset included:
...
###Ridge Regression
Ridge regression was performed on the dataset. cross validation was performed on the training data set to determien the optimum lambda for the ridge regression. This lambda was then using in a ridge regression model with the testing dataset.
