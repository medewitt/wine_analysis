...
fit5 <-lm(quality ~ alcohol, data = red_wine_data)
par(mfrow=c(2,2))
plot(fit5, main = "Alcohol")

fit6 <-lm(quality ~ volatile_acidity, data = red_wine_data)
par(mfrow=c(2,2))
plot(fit6, main = "Alcohol")

fit7 <-lm(quality ~ pH, data = red_wine_data)
par(mfrow=c(2,2))
plot(fit7, main = "pH")

fit8 <-lm(quality ~ citric_acid, data = red_wine_data)
par(mfrow=c(2,2))
