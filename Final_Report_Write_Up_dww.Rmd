...
In this document we will report on proposed statistical models for classification of red wine based on 11 predictors. The purpose of this analysis is to provide a model to vintners to better predict the quality rating for their product. Analysis will be performed using both regression and classification techniques.

# Description of Data
The data set provided is the Wine dataset from UC Irvine of red _vinho verde_ wine samples, from the north of Portugal [Cortez et al., 2009]. It consists of `r nrow(red_wine_data_raw) ` observations with a total of `r ncol(red_wine_data_raw) -1` physicochemical predictors and a response variable, Quality. The predictors include the following: `r names(red_wine_data_raw)`. The quality feature represents the judgment of an individual wine's quality, and is calculated by averaging the 1-10 point subjective judgments of taste-testers. Quality is the feature of interest for the dataset, as the vintner is interested in predicting a wine's subjective judged quality using objective measures. A summary of these measures as well as the response variable can be seen in Table 1. 
...
The distribution of these criteria can be seen below in the histograms in Figure 1.
...
Studentized residual and leverage plots of free sulfur dioxide reveal two observations with extraordinary values that should be considered for removal in the modeling process. These wines are observations 1080 and 1082.
...
These two wines have been removed from the clean dataset to allow for more accurate predictions from the remaining observations. The presence of these two wines may result in incorrect or inaccurate predictions. As we did not gather this dataset, we do not know if this information was incorrectly captured or if these values are real. However, given the strong indication that these two points are outliers with high leverage it is good practice to remove these two points.

# Method
To estimate the test error of our generated models for comparison, we separated the data into a training data set used to train the models and a test data set used to estimate the testing errors. Seventy percent of the raw data was randomly selected and placed in the training set with the remaining thirty percent used as the testing data set.

## Regression

To select the best fit regression model, we tried using several regression modeling methods: Least Squares Regression, Ridge Regression, Lasso Regression, Principle Components Regression, Partial Least Squares Regression, and Boosting. In each regression the dependent variable was the integer value of Quality. The data were divided into two sets, a training set to train the model and a testing set for model validation. We will now go deeper in the model generation process for each of these modeling types and methods.

### Least Squares
We selected the best least squares regression model via best subset selection. To determine the best subset, model, we first ran cross-validation on the training set to determine the best number of predictors to include in the model. This analysis indicated that any additional predictor after three variables did not increase the accuracy of the model. The training data were then used to determine the best subset of the linear model with three predictors. The best subset included volatile acidity, sulphates, and alcohol.
...
### Ridge Regression
Next we performed Ridge regression. We conducted cross-validation on the training data set to determine the optimal value for lambda for the ridge regression. This lambda, `r round(bestlam_ridge,3)`, was then used in a ridge regression model with the testing dataset. Notably, the ridge regression reveals that one coefficient stands out as a very large, with 11 much smaller coefficients.

### Lasso Regression
Next we performed LASSO regression with cross-validation. We conducted cross-validation to determine the optimal value for lambda for the LASSO regression. This value was `r round(bestlam_lasso,3)`. As a function of the LASSO regression, only pH was shrunk to zero; total sulphur dioxide and free sulphur dioxide were shrunk to near zero. As was the case with ridge regression, there appears to be one large coefficient compared to the others.
...
### Boosted Regression
Finally we performed Boosted regression. The interaction depth was limited to four in order to reduce the likelihood of over-fitting the data. The model was trained on 5,000 different trees and we assumed a Guassian distribution.
...
### Model Selection
The resulting mean squared errors for each regression method were tabulated to identify a superior model.
...
All of these models seek to maximize the global accuracy of the model. More interesting for the vintners is the ability to detect each of the three classes of wine quality.
...
# Discussion
This analysis shows that for regression the boosted model resulted in the highest accuracy of all regression models; however, this accuracy comes at a cost of interpretability. Because the boosted algorithms have little intrepretation this accuracy is more beneficial for prediction than inference. If inference is the goal for the vintner and horticulturalists who seek to understand the underlying properties that produce good wines, the LASSO regression model is preferred: it has greater interpretability and the second highest accuracy. While the PLS is more accurate, again it suffers from poor interpretability.

With this in mind, the best model for inference with high accuracy is characterized by the equation below:
...
The best model for prediction is the boosted regression model. An example tree is show below:
...
Turning to the classification method, the best overall classification method was Quadratic Discriminate Analysis. This is seen in both the overall accuracy as well as in its ability to accurately classify each subcategory. While the other methods have lesser abilities to detect the good and poor quality wines, the QDA method showed the best accuracy in these two fields. This is very important for vintners when it comes to pricing and selling. The penalty of misclassifying a good wine as medium or a poor wine as medium/good could potentially be severe, damaging the reputation of the winery. From this analysis it is clear that QDA is the superior method for classification of red wines in this dataset. The priors and group means for the QDA are shown in the preceding figures.
